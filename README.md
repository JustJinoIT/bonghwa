...
[![verify-windows](https://github.com/JustJinoIT/bonghwa/actions/workflows/verify-windows.yml/badge.svg)](https://github.com/JustJinoIT/bonghwa/actions/workflows/verify-windows.yml)
...
GitHub Actions Windows 러너에서 push→build→deploy→실패경로→rollback 전체
사이클을 자동 검증한다 (`.github/workflows/verify-windows.yml`).
...
- `scripts/install.ps1` — 사전점검(MSBuild/git/sh.exe/IIS) + 디렉토리 준비 + IIS Application 등록 + hook 설치
...
1. `scripts/bonghwa.config.ps1` 경로 수정 (MSBuild 경로 등 — 안 맞으면 `install.ps1`이 vswhere로 탐색해 안내)
2. `scripts/install.ps1 -GiteaRepoPath <bare repo 경로>` 실행 — 사전점검 + 디렉토리 준비 + IIS Application 등록 + hook 설치를 한 번에
3. push
