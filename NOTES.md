...
- [x] **push-트리거 배포 vs 수동 rollback 동시 실행 — 실제 경합 재현 + 수정 효과 실증 완료 (2026-07-11/12).**
  1차 시도(`VERIFY 6/6`, 300ms 지연)는 git credential-manager의 host-provider
  자동탐지 지연(~4초)으로 실제로는 안 겹치고 순차 실행됐다 ("겹쳤을 때도
  안전하다"가 아니라 "이번엔 안 겹쳤다"였음, 정직하게 기록).
  2차 시도에서 두 가지로 재설계해 실제 경합을 만들어냄:
  - `VERIFY 6/7`: dev clone에 `credential.helper ""`로 지연 원인을 제거하고
    타임스탬프로 겹침을 직접 증명. 실측: rollback `10:58:26.642~27.099` vs
    push(배포) `10:58:26.885~29.015` — **214ms 실제 겹침 확인.**
  - `VERIFY 7/7` (신규, 결정론적): git 타이밍에 기대지 않고 실제 `rollback.ps1`
    코드를 4개 병렬 잡으로 3초간 반복 실행(16회 호출)해 같은 `LiveJunction`에
    강제 경합. **16회 중 5회가 실제로 mklink 경합 실패(`mklink exit 1`)** —
    즉 코드 리뷰로 의심했던 레이스가 실측으로 재현된 진짜 버그였음. 이 5회
    전부 새로 고친 `rollback.ps1`(아래 버그 항목) 덕분에 `ROLLBACK_FAIL`로
    정확히 기록됨 — **고치기 전이었다면 이 5회 모두 "[rollback] DONE"으로
    조용히 거짓 성공 보고했을 상황.** 경합 후 최종 junction은 항상 유효한
    대상을 가리킴 (복구 불능 상태는 발생 안 함).
...
- **.ps1 파일에 UTF-8 BOM 없으면 Windows PowerShell 5.1이 한글/화살표를 깨뜨림**
  — 전체 .ps1에 BOM 추가.
- **`rollback.ps1`이 `mklink` 종료 코드를 확인하지 않음 — 코드리뷰로 의심 →
  실측으로 재현 확인 (2026-07-12).** `deploy.ps1`은 junction 스위칭 후
  `$LASTEXITCODE`를 체크해 실패 시 throw하는데 `rollback.ps1`은 같은 자리에
  체크가 없어서, mklink가 경합 등으로 실패해도 `[rollback] DONE`을 그대로
  찍고 이력에도 ROLLBACK을 정상 기록할 수 있었다 (실제 junction은 안
  바뀌었는데 성공했다고 보고하는 조용한 실패). `VERIFY 7/7` 결정론적
  스트레스 테스트(4개 병렬 rollback, 3초, 16회 호출)에서 **실제로 5회
  재현됨** — 수정 후에는 이 5회 전부 `ROLLBACK_FAIL`로 정확히 기록됨.
  exit code 체크 + 실패 시 `ROLLBACK_FAIL` 기록(`deploy.ps1`의 `DEPLOY_FAIL`과
  동일 원칙)으로 수정.
