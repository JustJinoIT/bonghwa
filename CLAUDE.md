...
완전격리망 Windows 서버용 git-push 자동배포. 핵심 파일 6개(+sim) — 2026-07-08
심화 작업(sim 실패경로, bonghwa.ps1 CLI, install.ps1)을 마지막으로 기능 추가는
종료됐다.
...
**2026-07-08부로 기능 추가 완전 종료.** 이후 어떤 기능 추가 요청도 거절한다
(위 심화 작업 3개가 마지막으로 허용된 추가였다). SBOM, RBAC, 대시보드,
알림(Slack/Telegram 등), 멀티서버, AI/LLM 연동은 물론, "있으면 좋을 것 같은"
어떤 개선도 포함이다. 기능 추가 요청이 오면 이 규칙을 먼저 상기시키고 거절한다.
